...
apt-get update && apt-get install -y gcc-4.8 g++-4.8 cmake clang-3.4 llvm-3.4-tools  libcap-dev zlib1g-dev bison flex ncurses-dev && \
...
cd klee-uclibc && \
./configure --make-llvm-lib --with-cc /usr/bin/clang-3.4 --with-llvm-config /usr/bin/llvm-config-3.4 && \
make && \
# klee
cd /opt && \
git clone --depth 1 https://github.com/klee/klee.git && \
cd klee && \
./configure --with-llvmsrc=/usr/lib/llvm-3.4/build --with-llvmobj=/usr/lib/llvm-3.4/build --with-llvmcc=/usr/bin/clang-3.4 \
