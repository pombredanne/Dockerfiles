...
apt-get update && apt-get install -y gcc-4.8 g++-4.8 cmake clang-4.0 llvm-4.0-tools  libcap-dev zlib1g-dev bison flex ncurses-dev && \
update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-4.8 20 && \
update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-4.8 20 && \
update-alternatives --install /usr/bin/clang clang /usr/bin/clang-4.0 20 && \
update-alternatives --install /usr/bin/clang++ clang++ /usr/bin/clang++-4.0 20 && \
...
cd klee-uclibc && \
./configure --make-llvm-lib --with-cc /usr/bin/clang-4.0 --with-llvm-config /usr/bin/llvm-config-4.0 && \
make && \
# klee
cd /opt && \
git clone --depth 1 https://github.com/klee/klee.git && \
cd klee && \
./configure --with-llvmsrc=/usr/lib/llvm-4.0/build --with-llvmobj=/usr/lib/llvm-4.0/build --with-llvmcc=/usr/bin/clang-4.0 \
  --with-llvmcxx=/usr/bin/clang++-4.0 --with-stp=/opt/stp/build --with-uclibc=/opt/klee-uclibc && \
